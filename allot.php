...
if(!isset($_GET['tutor']) || isset($_GET['tutor']) ==''){
    header('location:tutors.php?error=View details of a tutor to allocate students.');
}
?>
<!DOCTYPE HTML>
<html>
		...
		<!-- Main -->
			<div id="preview" class="vertical" style="border:2px solid gray; width:70%; margin:auto; border-radius:25px;">
				...
				<div class="inner">
					...
					<div class="image fit">
					    <hr>
					    <h2>Allocate students to Tutor <?php echo $_GET['tutor']; ?></h2>
						 <?
    //check for errors
    ...
    ?>
  <form action="php/process_allot.php" method="POST">
      <input type="text" name="tutor" value="<?php echo $_GET['tutor']; ?>" readonly><br>
      
      CTRL + SELECT to select multiple students to allocate.<br>
      <select name="students[]" multiple required>
          <option value="" hidden selected>Select students</option>
          	<?php
      ...
      <br>
       ...
       <input type ="submit" name="allot" class="button big alt" style="border:1px solid black;" value="Allocate" required><br>
